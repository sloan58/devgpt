    ...
    <x-authentication-card>
        <x-slot name="logo">
            <div class="flex">
                ...
                <p class="text-4xl font-semibold text-gray-400">DevGPT</p>
            </div>
        </x-slot>
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div>
                <x-label for="email" class="!text-gray-200" value="{{ __('Email') }}" />
                <x-input id="email" class="block mt-1 w-full " type="email" name="email" :value="old('email')" required autofocus autocomplete="username" />
            </div>

            <div class="mt-4">
                <x-label for="password" class="!text-gray-200" value="{{ __('Password') }}" />
